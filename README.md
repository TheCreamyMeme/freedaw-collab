...
Node 20 Alpine image handling Express, CORS, JWT, Socket.io, and Multer.
...
GET /api/users & PUT /api/users/profile: Fetch users and update profiles.

POST /api/samples/upload/:sampleId: Upload audio samples.

GET /api/projects & POST /api/projects: Sync and retrieve project JSON files.

DELETE /api/projects/:projectId: Securely delete a project (restricted to owners).

GET /api/plugins & POST /api/plugins/upload: Retrieve and upload .js DSP plugins.